...
int findBestTimerCombination(int numPins, int index, int pins[], PinMap* pinTimers[]) {
    struct StackFrame {
        ...
        int i;
        int bestScore;
        std::vector<PinMap*> searchArray;
    };

    std::stack<StackFrame> stack;
    stack.push(StackFrame{index, -1, NOT_FOUND, std::vector<PinMap*>(numPins)});

    int overallBestScore = NOT_FOUND;

    while (!stack.empty()) {
        StackFrame& current = stack.top();
        current.i++;

        int startIndex = findIndexOfFirstPinMapEntry(pins[current.index]);
        int endIndex = findIndexOfLastPinMapEntry(pins[current.index]);

        if (startIndex == -1 || endIndex == -1) {
            SIMPLEFOC_DEBUG("STM32-DRV: ERR: no timer on pin ", pins[current.index]);
            return -1; // pin is not connected to any timer
        }

        if (current.i > endIndex) {

            if (current.bestScore < overallBestScore && current.bestScore != NOT_FOUND) {
              // SIMPLEFOC_DEBUG("STM32-DRV: end ");
                overallBestScore = current.bestScore;
                for (int j = 0; j < numPins; j++) {
                    pinTimers[j] = current.searchArray[j];
                }
            }
            stack.pop();
            continue;
        ...
        if (current.i < startIndex) {
            current.i = startIndex;
        }

        current.searchArray[current.index] = (PinMap*)&PinMap_TIM[current.i];
        int score = NOT_FOUND;

        if (current.index < numPins - 1) {
            stack.push(StackFrame{current.index + 1, -1, NOT_FOUND, current.searchArray});
        } else {
            score = scoreCombination(numPins, current.searchArray.data());
            #ifdef SIMPLEFOC_STM32_DEBUG
            printTimerCombination(numPins, current.searchArray.data(), score);
            #endif

            if (score >= 0 && (score < current.bestScore || current.bestScore == NOT_FOUND)) {
                current.bestScore = score;
            }
        }
    }
    // SIMPLEFOC_DEBUG("STM32-DRV: best ",overallBestScore);
